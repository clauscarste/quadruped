...
t1 = Thread(target=main.startanimation)
t2.start()
#t1.start()

##      ##
xml_path = 'simulation_files/full_quadruped_simulation/scene.xml'
...
def controller(model, data):
    ...
    for motor_number_i in motor_number:
        set_position_servo(2*motor_number_i, kp)
        set_velocity_servo(1+2*motor_number_i, kv)
        toruqe = [data.sensordata[i] for i in [8, 11, 14, 23, 26, 29, 38, 41, 44,53,56,59]]
        if state[motor_number_i] == 1  and abs(toruqe[motor_number_i]) < 6:
            data.ctrl[2*motor_number_i] = can_comunication.position_setpoint[motor_number_i]
...
while not glfw.window_should_close(window):
    ...
    while (data.time - time_prev < 1.0/20.0): #the simulation runs at 20Hz
