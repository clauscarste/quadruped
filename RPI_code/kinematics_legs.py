...
def inverse_kinematics_legs(leg_id,x,y,z,leg_parameters,ofset,limit,invert_axis):
    ...
    motor_angle[0] = math.degrees(motor_angle[0])#+10
    ...
    #for i in range(3*leg_id, 3+(3*leg_id)): #takes 3 of the 12motors that belong to the leg id
     #   can_comunication.move_to(i, math.degrees(motor_angle[i-3*leg_id]), ofset[i],limit[i],invert_axis[i]) #i-3*leg_id always gives 0,1,2 what equals the first secound and third entry from motor_angle


inverse_kinematics_legs(2,-1,5,0,leg_parameters,motor_ofset,angle_limit,invert_axis)
